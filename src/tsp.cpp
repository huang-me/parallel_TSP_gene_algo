...
bool Genetic::_findInitial(vector<int> &arr, int curr, vector<bool> &visited, int &visitedNum) {
	arr.push_back(curr);
	visited[curr] = true;
	visitedNum++;
...
void Genetic::crossOver(vector<int> &parent1, vector<int> &parent2) {
	...
	int point1 = mystate->rand() % (graph->V - 1) + 1;
	int point2 = mystate->rand() % (graph->V - point1) + point1;
	...
	if (point1 == point2) {
		if (point1 - 1 > 1)
			...
			int point = mystate->rand() % 10 + 1; // number in the range 1 to 10
			if (point <= 5)
				point1--;
			else
				point2++;
	...
	// mutation
	int mutation = mystate->rand() % 100 + 1; // random number in [1,100]
	if (mutation <= mutation_rate) // checks if the random number <= mutation rate
	{
		...
		index_gene1 = mystate->rand() % (graph->V - 1) + 1;
		index_gene2 = mystate->rand() % (graph->V - 1) + 1;
		...
		if (real_size_population >= 2) {
			if (real_size_population == 2) {
				...
				do {
					...
					parent1 = mystate->rand() % real_size_population;
					parent2 = mystate->rand() % real_size_population;
